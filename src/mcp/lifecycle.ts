...
import { createFileIndexStore } from './indexStore';
import { ObjectIndex } from './objectIndex';
import { RunningMcpServer, startMcpServer } from './server';
import { createTaskQueue } from './taskQueue';
import { ALL_TOOLS } from './tools/registry';
import { rebuildIndex } from './tools/searchTools';
import { DEFAULT_TIMEOUTS, ToolDeps } from './tools/toolDefinition';
...
export function registerMcp(context: vscode.ExtensionContext): void {
  ...
  const bridge = createAdtBridge(log);
  ...
  async function ensureToken(): Promise<string> {
    ...
    token = stored ?? randomBytes(32).toString('base64url');
    if (!stored) await context.secrets.store(TOKEN_SECRET_KEY, token);
    return token;
  }
  ...
  async function rebuildIndexCommand(): Promise<void> {
    ...
    await vscode.window.withProgress(
      ...
      async (_progress, cancellation) => {
        try {
          const result = await rebuildIndex(deps, destination, undefined, () => cancellation.isCancellationRequested);
