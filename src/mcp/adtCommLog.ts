...
const LINE_PATTERN =
  /^(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})\.(\d{3}) \[\w+\] (\S+)\s+POST\s+(\d{3})\s.*[?&]_action=(LOCK|UNLOCK)(?:&|\s*$)/;

export type LockAction = 'LOCK' | 'UNLOCK';

export interface AdtCommReader {
  // HTTP status of the first LOCK/UNLOCK call to `destination` logged at or
  // after sinceMs, polled for up to waitMs; undefined when none showed up.
  lockStatusSince(action: LockAction, destination: string, sinceMs: number, waitMs: number): Promise<number | undefined>;
}
...
export function parseLockStatus(text: string, action: LockAction, destination: string, sinceMs: number): number | undefined {
  for (const line of text.split(/\r?\n/)) {
    const match = LINE_PATTERN.exec(line);
    if (!match || match[10] !== action || match[8].toUpperCase() !== destination.toUpperCase()) continue;
...
export function createAdtCommReader(logPath: string | undefined, pollMs = 150): AdtCommReader {
  return {
    async lockStatusSince(action, destination, sinceMs, waitMs): Promise<number | undefined> {
      if (!logPath) return undefined;
      const deadline = Date.now() + waitMs;
      for (;;) {
        try {
          const status = parseLockStatus(await readTail(logPath), action, destination, sinceMs);
